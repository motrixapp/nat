...
} from './helpers/docker-harness.js'
...
describe.skipIf(SKIP)('NAT docker matrix', () => {
  ...
  // Docker Desktop/OrbStack run host networking inside a Linux VM, so SSDP
  // multicast is only observable from the test process on a Linux host.
  describe.skipIf(process.platform !== 'linux')('UPnP IGD v1', () => {
