import type { ParseResult } from './codecs/parse-result.js'
import { parseErr, parseOk } from './codecs/parse-result.js'
...
export class PortChecker {
  ...
  async checkPortReachable(
    ...
    if (options.endpoints.length === 0) {
      return parseErr(
        NatErrorCode.Timeout,
        'no port-checker endpoints configured'
      )
    }
    if (options.endpoints.length > MAX_ENDPOINTS) {
      return parseErr(NatErrorCode.Timeout, 'too many port-checker endpoints')
    }
    const nonHttps = options.endpoints.find((e) => !e.startsWith('https://'))
    if (nonHttps) {
      return parseErr(NatErrorCode.SecurityViolation, 'endpoint must use HTTPS')
    }
    ...
    return parseErr(NatErrorCode.Timeout, 'all port-checker endpoints failed')
  }
}
