...
import { ALLOWED_SERVICE_TYPES_FOR_PORT_MAPPING } from './soap-codec.js'
import {
  findChild,
  findDescendants,
  localName,
...
export function parseDeviceDescription(
  ...
  if (localName(root.name) !== 'root') {
    return parseErr(NatErrorCode.ParseError, 'root element is not <root>')
  }

  const topDevice = findChild(root, 'device')
  if (!topDevice) {
    return parseErr(NatErrorCode.ParseError, 'missing device element')
  }
...
function validateControlUrl(url: string): ParseResult<string> {
  if (url.length === 0 || url.length > CONTROL_URL_MAX_LENGTH) {
    return parseErr(NatErrorCode.SecurityViolation, 'controlURL length')
  }
  if (!url.startsWith('/')) {
    return parseErr(
      NatErrorCode.SecurityViolation,
  ...
  if (url.includes('..')) {
    return parseErr(NatErrorCode.SecurityViolation, 'controlURL contains ..')
  }
  return parseOk(url)
}
