...
import { isValidPort } from './ip-utils.js'
...
export function parsePcpMapResponse(
  ...
  if (sourceIp !== gatewayIp) {
    return parseErr(NatErrorCode.SecurityViolation, 'pcp source IP mismatch')
  }
  if (
  ...
  if (expectedNonce.length !== PCP_NONCE_LEN) {
    return parseErr(NatErrorCode.ParseError, 'expectedNonce must be 12 bytes')
  }
  ...
  if ((rawOpcode & 0x80) === 0) {
    return parseErr(NatErrorCode.ParseError, 'pcp response bit not set')
  }
  if ((rawOpcode & 0x7f) !== PCP_OPCODE_MAP) {
    return parseErr(NatErrorCode.ParseError, 'pcp opcode mismatch')
  }
  ...
  if (!nonceOk) {
    return parseErr(NatErrorCode.SecurityViolation, 'pcp nonce mismatch')
  }
