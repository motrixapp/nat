...
import { isIpv4String, isLinkLocalIpv4, isPrivateIpv4 } from './ip-utils.js'
...
export function parseMSearchResponse(raw: Buffer): ParseResult<SsdpResponse> {
  if (raw.length > SSDP_MAX_RESPONSE_SIZE) {
    return parseErr(NatErrorCode.SecurityViolation, 'ssdp response too large')
  }
  ...
  for (const b of raw) {
    ...
    if (b < 0x20 && b !== 0x09 && b !== 0x0a && b !== 0x0d) {
      return parseErr(NatErrorCode.SecurityViolation, 'control byte in ssdp')
    }
  }
  ...
  if (lines.length === 0) {
    return parseErr(NatErrorCode.ParseError, 'empty response')
  }
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    if (colon <= 0) return parseErr(NatErrorCode.ParseError, 'malformed header')
    const name = line.slice(0, colon).trim().toUpperCase()
    const value = line.slice(colon + 1).trim()
    if (name.length > SSDP_HEADER_NAME_MAX) {
      return parseErr(NatErrorCode.SecurityViolation, 'header name too long')
    }
  ...
  if (!location) return parseErr(NatErrorCode.ParseError, 'missing LOCATION')
  if (location.length > SSDP_LOCATION_MAX_LENGTH) {
    return parseErr(NatErrorCode.SecurityViolation, 'location too long')
  }
...
export function validateLocationUrl(url: string): ParseResult<{
  ...
  if (!url.startsWith('http://')) {
    return parseErr(NatErrorCode.SecurityViolation, 'location must be http://')
  }
  const rest = url.slice('http://'.length)
  if (rest.includes('@')) {
    return parseErr(NatErrorCode.SecurityViolation, 'userinfo forbidden')
  }
  if (rest.includes('#')) {
    return parseErr(NatErrorCode.SecurityViolation, 'fragment forbidden')
  }
  if (rest.includes('?')) {
    return parseErr(NatErrorCode.SecurityViolation, 'query forbidden')
  }
  ...
  if (colonIx >= 0) {
    ...
    if (!/^\d{1,5}$/.test(portStr)) {
      return parseErr(NatErrorCode.ParseError, 'invalid port')
    }
    port = Number(portStr)
    if (port < 1 || port > 65535) {
      return parseErr(NatErrorCode.ParseError, 'port out of range')
    }
  } else {
    host = authority
  }
  ...
  if (!isIpv4String(host)) {
    return parseErr(NatErrorCode.SecurityViolation, 'host must be literal IPv4')
  }
