...
export function parseSoapResponse(xml: string): ParseResult<SoapResult> {
  if (xml.length > SOAP_MAX_RESPONSE_SIZE) {
    return parseErr(NatErrorCode.SecurityViolation, 'soap response too large')
  }
  ...
  if (localName(envelope.name) !== 'Envelope') {
    return parseErr(NatErrorCode.ParseError, 'root is not s:Envelope')
  }

  const bodyEl = findChild(envelope, 'Body')
  if (!bodyEl) return parseErr(NatErrorCode.ParseError, 'missing s:Body')
  ...
  if (!actionEl) return parseErr(NatErrorCode.ParseError, 'empty body')
