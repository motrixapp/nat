...
import { isPublicIpv4 } from './ip-utils.js'
...
export function parseBindingResponse(
  ...
  if (buf.length < STUN_HEADER_SIZE || buf.length > STUN_MAX_RESPONSE_SIZE) {
    return parseErr(NatErrorCode.ParseError, `stun length: ${buf.length}`)
  }
  if (expectedTxId.length !== STUN_TXN_ID_SIZE) {
    return parseErr(NatErrorCode.ParseError, 'expectedTxId must be 12 bytes')
  }
  ...
  if (!safeEqual(buf.subarray(4, 8), cookieBuf)) {
    return parseErr(NatErrorCode.SecurityViolation, 'stun wrong magic cookie')
  }

  const txId = buf.subarray(8, 20)
  if (!safeEqual(txId, expectedTxId)) {
    return parseErr(NatErrorCode.SecurityViolation, 'stun txId mismatch')
  }
  ...
  while (offset < end) {
    ...
    if (offset + 4 > end) {
      return parseErr(NatErrorCode.ParseError, 'attribute header truncated')
    }
    const attrType = buf.readUInt16BE(offset)
    const attrLen = buf.readUInt16BE(offset + 2)
    if (offset + 4 + attrLen > end) {
      return parseErr(NatErrorCode.ParseError, 'attribute value exceeds bounds')
    }
  ...
  if (!result) {
    return parseErr(NatErrorCode.ParseError, 'no mapped address attribute')
  }
...
function parseMappedAddress(
  ...
  if (length < 8) {
    return parseErr(NatErrorCode.ParseError, 'mapped-address attr too short')
  }
