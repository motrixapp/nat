...
export function parseXml(
  ...
  for (const token of tokens) {
    if (token.type === XmlTokenType.StartTag) {
      ...
      if (elementCount > maxElements) {
        return parseErr(NatErrorCode.SecurityViolation, 'too many elements')
      }
      ...
      if (stack.length > 0) {
        ...
        return parseErr(NatErrorCode.ParseError, 'multiple root elements')
      }
      if (!token.selfClosing) stack.push(element)
    } else if (token.type === XmlTokenType.EndTag) {
      if (stack.length === 0) {
        return parseErr(NatErrorCode.ParseError, 'end tag without start')
      }
      const top = stack.pop()
  ...
  if (stack.length > 0) {
    return parseErr(NatErrorCode.ParseError, 'unclosed elements')
  }
  if (root === null) {
    return parseErr(NatErrorCode.ParseError, 'no root element')
  }
