...
import { createMockUdpFactory } from '../net/mock-udp-socket.js'
...
describe('PmpPcpClient', () => {
  ...
  it('times out when no response', async () => {
    ...
    if (!r.ok) expect(r.error).toBe(NatErrorCode.Timeout)
  })
  ...
  it('rejects concurrent NAT-PMP requests with NatProtocolRejected', async () => {
    ...
    if (!p2.ok) expect(p2.error).toBe(NatErrorCode.ProtocolRejected)
  ...
  it('rejects PCP requests over MAX_CONCURRENT_REQUESTS', async () => {
    ...
    expect(r.ok).toBe(false)
    if (!r.ok) expect(r.error).toBe(NatErrorCode.ProtocolRejected)
