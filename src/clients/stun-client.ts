...
import type { UdpSocketFactory } from '../net/udp-socket.js'
...
export class StunClient {
  ...
  async detectNatType(
    ...
    if (options.servers.length === 0) {
      return parseErr(
        NatErrorCode.StunDetectionFailed,
        'no STUN servers configured'
      )
    }
    if (options.servers.length > MAX_SERVERS) {
      return parseErr(NatErrorCode.StunDetectionFailed, 'too many STUN servers')
    }
    ...
    return parseErr(NatErrorCode.StunDetectionFailed, 'all STUN servers failed')
  }

  private parseServer(s: string): ParseResult<{ host: string; port: number }> {
    const m = /^([a-z0-9.-]+):(\d{1,5})$/i.exec(s)
    if (!m)
      return parseErr(NatErrorCode.ParseError, 'invalid STUN server format')
    ...
    if (port < 1 || port > 65535) {
      return parseErr(NatErrorCode.ParseError, 'STUN port out of range')
    }
  ...
  private queryServer(
    ...
    return new Promise<ParseResult<StunResult>>((resolve) => {
      ...
      const timer = setTimeout(
        () => void finish(parseErr(NatErrorCode.Timeout, 'stun timeout')),
        safeTimeout
      )
      timer.unref?.()
