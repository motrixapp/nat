...
import type { HttpClient } from '../net/http-client.js'
import type { UdpRemoteInfo, UdpSocketFactory } from '../net/udp-socket.js'
...
export class UpnpClient {
  ...
  async discover(
    ...
    return new Promise<ParseResult<DiscoveredGateway>>((resolve) => {
      ...
      const timer = setTimeout(async () => {
        if (!settled) {
          await settle(
            parseErr(NatErrorCode.DiscoveryFailed, 'discovery timed out')
          )
        }
      }, timeoutMs)
      timer.unref?.()
      ...
      ;(async () => {
        try {
          ...
          clearTimeout(timer)
          await settle(
            parseErr(NatErrorCode.DiscoveryFailed, (err as Error).message)
          )
        }
      })()
    })
  ...
  async getExternalIp(
    ...
    if (!ip) {
      return parseErr(NatErrorCode.ParseError, 'missing NewExternalIPAddress')
    }
    return parseOk(ip)
  }
...
function mapUpnpErrorCode(code: number | null): NatErrorCode {
  switch (code) {
    case 718:
      return NatErrorCode.MappingConflict // ConflictInMappingEntry
    case 725:
      return NatErrorCode.ProtocolRejected // OnlyPermanentLeasesSupported
    case 727:
      return NatErrorCode.ProtocolRejected // ExternalPortOnlySupportsWildcard
    default:
      return NatErrorCode.ProtocolRejected
  }
}
