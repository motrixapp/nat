  ...
  async request(input: HttpRequestInput): Promise<ParseResult<HttpResponse>> {
    ...
    return new Promise<ParseResult<HttpResponse>>((resolve) => {
      ...
      req.setTimeout(timeoutMs, () => {
        req.destroy()
        settle(
          parseErr(NatErrorCode.Timeout, `http timeout after ${timeoutMs}ms`)
        )
      })

      req.on('error', (e) => {
        settle(parseErr(NatErrorCode.GatewayUnreachable, e.message))
      })

      const onAbort = () => {
        req.destroy()
        settle(parseErr(NatErrorCode.Timeout, 'aborted'))
      }
      if (input.signal) {
